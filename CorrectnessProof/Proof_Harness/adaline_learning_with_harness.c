...
void proof_harness_adaline_predict() {
  ...
  double x[] = {1.0, 2.0};
  double out;
  int pred = adaline_predict(&ada, x, &out);
  assert(pred == 1);
  assert(out == 8.0);
  free(ada.weights);
}
...
void proof_harness_adaline_fit_sample() {
  ...
  double x[] = {1.0, 2.0};
  int y = 1;
  double correction_factor = adaline_fit_sample(&ada, x, y);
  assert(ada.weights[0] == 1.1);
  assert(ada.weights[1] == 2.2);
  assert(ada.weights[2] == 3.1);
  assert(correction_factor == 0.1);
  free(ada.weights);
}
